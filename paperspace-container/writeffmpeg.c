...
static int start_ffmpeg_impl(FFMpegContext *context, struct RenderData *rd, int rectx, int recty, const char *suffix, ReportList *reports)
{
    ...
    // comment out this for RTMP as there is no file output and latter on end_ffmpeg_impl deinit it.
    ...
    // Handle this with rtmp aspect later
    // check if the path contains rtmp:// then avoid setting this.
    if( strstr(name, "rtmp") == NULL) {
	    of = avformat_alloc_context();
        of->oformat = fmt;
    }
    else {
        avformat_alloc_output_context2(&of, NULL, "flv", name);
    }
	if (!of) {
...
static void ffmpeg_filepath_get(FFMpegContext *context, char *string, RenderData *rd, bool preview, const char *suffix)
{
    ...
    // check if path contains rtmp
    if (strstr(string, "rtmp://") == NULL) {
...
static void end_ffmpeg_impl(FFMpegContext *context, int is_autosplit)
{
    ...
    // Delete when including UI options in
